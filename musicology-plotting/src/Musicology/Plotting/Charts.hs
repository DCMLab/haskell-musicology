...
pianoRollStyle color =
  (solidFillStyle color, Just $ solidLine 1.0 $ opaque black)

ptod :: (Interval p, ToMidi p) => Pitch p -> Double
ptod = fromIntegral . toMidi . toInterval

plotPianoRoll
  :: (HasPitch n, HasTime n, PlotValue (TimeOf n), ToMidi (IntervalOf n))
  => PianoRollPlot n
  -> Plot (TimeOf n) Double
